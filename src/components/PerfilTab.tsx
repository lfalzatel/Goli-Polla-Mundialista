  ...
  return (
    <div className="space-y-6">
      ...
      {email === 'lfalzatel@gmail.com' && (
        <section className="bg-amber-50 border border-amber-200 rounded-2xl p-6 shadow-sm space-y-4">
          <h3 className="font-display text-2xl text-amber-900 flex items-center gap-2 uppercase tracking-wide">
          ...
          <p className="text-amber-800 text-sm font-sans">
            Estas opciones solo son visibles para ti como administrador.
          </p>
          <div className="flex gap-3">
            <button
              onClick={handleSeedPartidos}
              className="bg-amber-600 hover:bg-amber-700 text-white font-sans text-sm font-bold py-3 px-5 rounded-xl transition-all shadow-md active:scale-95 flex items-center gap-2"
            >
              <span className="material-symbols-outlined text-[18px]">database</span>
              Inicializar Base de Partidos
            </button>
          </div>

          <div className="mt-6 pt-4 border-t border-amber-200">
            <h4 className="font-sans font-bold text-amber-900 mb-2">Crear Nuevo Grupo</h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
              <input 
                ...
                className="w-full bg-white border border-amber-300 rounded-lg py-2 px-3 text-sm text-slate-800 font-semibold focus:ring-2 focus:ring-amber-500 outline-none"
              />
              <input 
                type="text" 
                ...
                className="w-full bg-white border border-amber-300 rounded-lg py-2 px-3 text-sm text-slate-800 font-semibold focus:ring-2 focus:ring-amber-500 outline-none uppercase"
              />
            </div>
            <button
              onClick={handleCreateGroup}
              className="bg-[#034226] hover:bg-[#02331d] text-white font-sans text-sm font-bold py-2.5 px-5 rounded-xl transition-all shadow-md active:scale-95 w-full md:w-auto"
            >
              Crear Grupo
            </button>
            {groupCreateMsg && <p className="text-amber-800 text-xs font-bold mt-2">{groupCreateMsg}</p>}
          </div>

          {/* Lista de Grupos Creados */}
          {misGrupos.length > 0 && (
            <div className="mt-6 pt-4 border-t border-amber-200">
              <h4 className="font-sans font-bold text-amber-900 mb-3 flex items-center gap-2">
                <span className="material-symbols-outlined text-[18px]">group</span>
                Tus Grupos Creados
              </h4>
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
                {misGrupos.map(g => (
                  <div key={g.id} className="bg-white border border-amber-300 rounded-lg p-3 flex justify-between items-center shadow-sm">
                    <div>
                      <p className="font-bold text-slate-800 text-sm leading-tight">{g.nombre}</p>
                      <p className="text-[10px] text-slate-500 uppercase tracking-widest font-semibold mt-0.5">Activo</p>
                    </div>
                    <button 
